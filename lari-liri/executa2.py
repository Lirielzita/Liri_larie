...
def incluir_respirador():
    ...
    dados = request.get_json()
    try: # tentar executar a operação
      nova = Respirador(**dados) # criar o novo exame realizado
      db.session.add(nova) # adicionar no BD
      db.session.commit() # efetivar a operação de gravação
    except Exception as e: # em caso de erro...
      # informar mensagem de erro
...
def listar_respiradores():
    # obter exames realizados
    listar_respiradores = db.session.query(Respirador).all()
    # converter dados para json
    lista_jsons = [ x.json() for x in listar_respiradores ]
    # converter a lista do python para json
    resposta = jsonify(lista_jsons)
    # PERMITIR resposta para outras pedidos oriundos de outras tecnologias
    resposta.headers.add("Access-Control-Allow-Origin", "*")
...
#  incluir_respirador, listar_respiradores; incluir médico (novo atributo: CRM) que solicitou o exame 